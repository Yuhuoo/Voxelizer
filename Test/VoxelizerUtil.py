...
def voxelize(mesh_v, mesh_f, dim_h, dim_w, voxel_size):
    box_half_size = np.array([voxel_size/2.0, voxel_size/2.0, voxel_size/2.0])  # 56
    dim_x, dim_y, dim_z = dim_w, dim_h, dim_w   # 128, 128, 128
    dim_x_half, dim_y_half, dim_z_half = dim_x / 2, dim_y / 2, dim_z / 2    # 56, 56, 56
                ...
                for zz in range(z_dim):
                    if (v_volume[xx, yy, zz, :] != np.zeros((3,), dtype=np.float32)).any():
                        ...
                        fp.write('v %f %f %f %f %f %f\n' % \
                                 (pt[0], pt[1], pt[2], \
                                  v_volume[xx, yy, zz, 0], v_volume[xx, yy, zz, 1], v_volume[xx, yy, zz, 2]))
